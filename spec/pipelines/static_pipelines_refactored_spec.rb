...
describe 'static concourse pipelines spec' do
  ...
  context 'resource_type exists' do
    ...
    let(:pipeline_files) { Dir.glob(pipelines_dir + '**/*.yml') } #+ Dir.glob(pipelines_dir + '**/*.erb') }
    ...
    let(:resource_types_object) do
      result = []
      puts "list: #{pipeline_files}"
      pipeline_files.each do |pipeline_filename|
        puts "processing file #{pipeline_filename}"
        pipeline = ConcoursePipeline.load_file(pipeline_filename, pipeline_filename).load
        puts pipeline.resource_types
        result.concat(pipeline.resource_types.to_a) unless pipeline.resource_types.empty?
      end
      result.uniq
    end
